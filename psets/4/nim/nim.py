...
class NimAI:
    ...
    def update_q_value(
        self, state: list[int], action: tuple[int, int], old_q, reward, future_rewards
    ):
        """
    ...
    def best_future_reward(self, state: list[int]):
        ...
        # If there is no possible actios, return 0
        ...
        for i, j in possible_actions:
            ...
            # If reward does not exists, defaults to 0
            if reward is None:
                reward = 0
    ...
    def choose_action(self, state: list[int], epsilon=True):
        ...
        for action in possible_actions:
            key = (tuple(state), action)
            action_reward = self.q.get(key)

            # Get the action with the best future (leads to more rewards)
